...
def test_report_progress_seed_data(tmp_path, capsys):
    ...
    assert "You have completed 0 of 100 skills (0 mastered, 0 passed) across 0 study sessions (0.0 hours)." in out
...
def test_report_progress_with_completed_nodes_and_sessions(tmp_path, capsys):
    ...
    assert "You have completed 2 of 100 skills (1 mastered, 1 passed) across 2 study sessions (2.0 hours)." in out
...
def test_report_resources_seed_data(tmp_path, capsys):
    ...
    assert "resource-report: 45 resource(s), 100 node(s)" in out
    assert "khan-arithmetic" in out
    assert "coverage: 100/100 node(s) have a linked resource." in out
