...
**Decisions:** [G-Preferences #219](https://github.com/earledotpy/skilltrace/issues/219) · [G-RouteSurface #220](https://github.com/earledotpy/skilltrace/issues/220) · [G-Direction #224](https://github.com/earledotpy/skilltrace/issues/224) · [G-JS #225](https://github.com/earledotpy/skilltrace/issues/225) · [G-Slot #226](https://github.com/earledotpy/skilltrace/issues/226) · [G-MentorVoice #229](https://github.com/earledotpy/skilltrace/issues/229) · the seven audits [A-Today #209](https://github.com/earledotpy/skilltrace/issues/209) · [A-Next #210](https://github.com/earledotpy/skilltrace/issues/210) · [A-Node #211](https://github.com/earldotpy/skilltrace/issues/211) · [A-Health #212](https://github.com/earledotpy/skilltrace/issues/212) · [A-Analytics #213](https://github.com/earledotpy/skilltrace/issues/213) · [A-Modals #214](https://github.com/earledotpy/skilltrace/issues/214) · [A-Chrome #215](https://github.com/earledotpy/skilltrace/issues/215). ADR 0007 (amended), ADR 0006, `docs/spec-tier1-serve.md` §B/§C/§L-bis.
**Prototype (throwaway):** `prototype/p24-evolve.html` (P1 base + palette), `prototype/p24-radical.html` (the winning Today shape + verbs), `prototype/p24-reconsider.html` (the two borrows), `prototype/p25-graph-spine.html`, `prototype/p25-unified.html`. None ships; the engine never reads them.
**Build plan:** 5 `wayfinder:task` slices — S1 Interface seam → S2 Sublayer scaffold + chrome → S3 Unified home → S4 Next / node / finder → S5 Health / analytics / safety panels — in §I, the #62 five-slice pattern.
...
---
## Amendment — Interface re-direction (map #243)

This spec is amended by **map #243 — Interface re-direction: the single rich page** (charted 2026-09-14). The original locked direction (map #208) was re-opened because the owner judged the landed v2.4 UI (the P3 card-stack) a stripped-down version of the intended single rich page (`prototype/p24-reconsider.html`'s vision). The amendment was resolved through:

- **A-Landed [#244](https://github.com/earledotpy/skilltrace/issues/244)** — audit of the landed UI against the single-page intent
- **P-GraphSpine [#245](https://github.com/earledotpy/skilltrace/issues/245)** — prototype: reconsider graph home with the scorecard fix
- **P-Unified [#246](https://github.com/earledotpy/skilltrace/issues/246)** — prototype: the maximalist single-page home
- **G-Preferences [#247](https://github.com/earledotpy/skilltrace/issues/247)** — amend the locked home-density and visibility rows (8 rows amended)
- **G-Direction [#248](https://github.com/earledotpy/skilltrace/issues/248)** — lock the amended single-page direction (unified home wins)
- **G-JS [#249](https://github.com/earledotpy/skilltrace/issues/249)** — re-open the interaction posture (prototype-gated): narrow tier 1 (chart hover/focus tooltips only)
- **G-Health [#251](https://github.com/earledotpy/skilltrace/issues/251)** — revise `/health` from engine check to learner-useful triage
- **G-Spec [#250](https://github.com/earledotpy/skilltrace/issues/250)** — this amendment + literal acceptance gates (DD1–DD6 updated)

**Key changes vs. original lock:**
- Home becomes the **unified single-page home** (hero + 6-card bento at dense 20/28 band) — not the P3 card-stack
- P1.1 → named-twice with pronoun focus; P1.2 → bounded backlog (ranked surfacing permitted); P2.1 → one blessed table; P5.2 → dense band 20/28/14/20; P5.3 → rich-home cap ≤8 blocks; §A ruled-out "spines as home IA" and "any bento" repealed under dominance test
- Interaction posture: **tier 1 narrow** (chart tooltips only), ADR 0008 written, ADR 0006 unamended, DD6 → per-route budget gate
- `/health` becomes attention-first triage with collapsed detail, learner-word rows, verdict line dropped from web
- Build slice S3: "Today card-stack" → "Unified home"
- Five throwaway prototypes now: `p24-evolve`, `p24-radical`, `p24-reconsider`, `p25-graph-spine`, `p25-unified`

---

## A. The design direction (G-Direction #224 · amended by G-Direction #248)

The v2.4 direction is **owner-decided** and locked here in full. It is a **hybrid**:

- **Base architecture — and every surface except the home — is P1-Evolve** (the current dashboard language, tightened): single column, airy, de-CLI-flavoured, **tier-0 zero JS**. Shell, nav, node detail, `/next`, the health roll-up, `/analytics`, the finder, the error shell and the pass/master panels all keep P1's shape. This is the direction a builder can implement without a single open UX question.
- **Home's shape is the unified single-page home** (P-Unified [#246](https://github.com/earledotpy/skilltrace/issues/246) as built): a **dominant hero focus** plus a **six-card bento** at the dense register — the maximalist single page the owner set out to reach. The home renders **seven card-level blocks** (hero + 6 bento; ≤ 8 per amended P5.3):

  1. **the hero — focus viewport** — focus title appears once as a heading; exactly one primary CTA on the page, pronoun + state-honest ("Start studying" at zero sessions; "Continue …" when resumable), never `Start <title>` literally (P1.1a/P1.1b as amended). Hero is double-weight by construction: full grid span, 30px display heading, 28px padding, accent left border.
  2. **the queue card** — ranked preview rows naming other nodes (ranked context, not focus repetition); no raw backlog dump on home. Full ranking lives on `/next`.
  3. **the pressure card** — one calm line (reviews due / blockers / overdue); blank-days-honestly at the zero state; never a pressure bit, count never in a refusal.
  4. **the spine card** — pronoun-headed ("what your focus opens"); one locator repetition of the focus name permitted here (named-twice). Spine-as-home-section permitted under the dominance test (one CTA, hero double-weight) — the old spine-as-IA ban is repealed, not ignored.
  5. **the week card** — strip + one line; blank-days-honestly; week paging marked WANTS-JS feeds G-JS.
  6. **the session-history card** — empty-state + format preview at zero sessions; sessions stay YAML-persisted, surfaced read-only.
  7. **the browse card** — the one blessed grouped-count table (P2.1 borrowing); browse filter marked WANTS-JS feeds G-JS. First cut if a future effort reinstates zero-tables — recorded here so the fallback is a list, not a redesign. **G-Health handoff triaged here:** the owner's ask for skill search showing a single line per node plus a short description of what it covers, grouped/categorized by how nodes relate (G-Health #251 out-of-lane finding) fits within the frozen route table as enhanced browse/finder content — the browse card's grouped table and the title-first finder on `/nodes/jump` are the surfaces; no route or table change required.

  **Bento discipline:** every bento card is links-only (14.5px, no second CTA anywhere) — R1's bento caveat passes in markup; the continue affordance is never one card among six.
...
**Ruled out (recorded, never to be silently resurrected — rewritten from §A):**
- P2's spines as home IA — **REPEALED under the dominance test**. Spine permitted as a home *section* when the continue affordance stays dominant (one CTA, hero double-weight as locked above). Spine as the home *organizing principle* (reconsider's `?v=graph` IA) stays out — the winner organizes by focus-first bento, not by spine.
- Any bento — **REPEALED under the same dominance test**. Bento permitted when every bento card is links-only and the hero is double-weight (R1 caveat passes in markup, demonstrated by both prototypes).
- P3's palette-first canvas as primary navigator — **KEPT**. No palette canvas navigation; §B tokens are fixed inputs.
- P3's all-receded home including Health — **KEPT**. Health stays its own route; [G-Health](https://github.com/earledotpy/skilltrace/issues/251) revises its content only, never folds it home.
- P1's rail — **KEPT** (stays ruled out). No rail.
- 1040px-daily-loop layout — **AMENDED**. The 1040px shell, 960px breakpoint, and tokens stand; the daily-loop single-column *Today shape* (card-stack ≤4) is superseded by the rich-home bento grid above for home only.
- Master purple `#7c3aed` — **KEPT**. One accent (terracotta) stands.
- Graph-spine home as built (P-GraphSpine [#245](https://github.com/earledotpy/skilltrace/issues/245): pronoun viewport + spine-carries-name, bounded day, 3 sections) — **judged coherent but set aside**: it leaves queue/pressure/week off home and cannot carry the maximalist destination.
- Landed v2.4 card-stack as built (incumbent control: focus card + count-set + `/next` pointer) — **judged faithful to the old spec but set aside**: the dissatisfaction is with the direction itself (A-Landed), and keeping it rejects the map destination.

### Today's focus CTA — the pronoun form

The focus viewport card carries **no title heading**; the CTA is pronoun + state-honest ("Start studying" at zero sessions; "Continue …" when resumable), never `Start <title>` literally (P1.1b as amended). The full-title form is unchanged everywhere else on the route surface.
...
Placement and honesty are already locked: one quiet line inside the hero's count context (G-Direction #248), plus a quiet factual line at session close (P1.7). A mirror, never a metronome — the count never appears in a refusal, a pressure bit, a ranking, or eligibility (`CONTEXT.md` — *Days practiced*).

---
## B. Palette, type, spacing tokens (G-Direction #224 · amended by G-Preferences #247 + G-Direction #248)
...
**Type scale.** Base `16px` humanist, line-height ~`1.55`; a display heading at ~**2× base (32px)** used on **the home's hero focus only** (P5.1); steps `24 / 14 / 13.5`px; prose measure **70ch** (the 65–75 band per P5.2).

**Spacing — two registers.** The **airy daily register** (non-home daily surfaces: `/next`, node brief, the P1-Evolve surfaces) uses the locked band: card padding `24–32px`; section gap `40px` (the 32–48 band) > intra-card gap `14px` (the 12–16 band) — **section gap > intra-card gap** is the locked invariant (P5.2). The **dense diagnostics register** (rich home, `/health` detail, `/analytics` tables, node drill-down) uses the dense band locked literally for G-Spec gates: **20px card padding / 28px section gap / 14px intra-card gap / 20px bento gutters**; section gap > intra-card gap preserved (28 > 14); prose ≤70ch. The rich-home cap is **≤8 blocks (hero + ≤7 cards), one CTA, ≤1 grouped-count table, disclosures off the primary path** (amended P5.3). Today (non-home) density cap per P5.3 remains ≤4 card-level blocks, one primary CTA, zero tables, no `<details>` on the primary path.
...
|---|---|---|---|
| P1.1a | **One focus, one action; named-twice with pronoun focus.** Exactly one primary CTA per view; the focus title appears once as a heading; one locator repetition (spine chip or pathway context) is permitted; queue rows naming other nodes are not focus repetition. The focus viewport card carries no title heading; the CTA is pronoun + state-honest ("Start studying" at zero sessions; "Continue …" when resumable), never `Start <title>` literally. | **Apply** (amended) | `spec` + `prototype` |
| P1.1b | **CTA is pronoun, state-honest.** The focus viewport card carries no title heading; the CTA is pronoun + state-honest ("Start studying" at zero sessions; "Continue …" when resumable), never `Start <title>` literally. Supersedes the `Start <title>` / `Continue <title>` wording. | **Apply** (amended) | `spec` |
| P1.2 | **Bounded backlog.** Beyond the focus, ranked/abstracted surfacing (queue rows, grouped counts, week strip) is permitted on home; the raw backlog never renders on home. The raw 49-flat-list dump never renders on home. | **Apply** (amended) | `spec` |
...
| # | Decision (locked wording) | Verdict | Binding |
|---|---|---|---|
| P2.1 | **Two density registers stand, one borrowing blessed.** Airy daily vs dense diagnostics stand, same tokens; one grouped-count table is permitted on the rich home as an explicit borrowing (the browse card). | **Apply** (amended) | `spec` |
...
| # | Decision (locked wording) | Verdict | Binding |
|---|---|---|---|
| P5.1 | **Type scale**: base `14px/1.5` → **16px humanist, ~1.55**; a display heading at **~2× base**, used on **the home's hero focus only**. | **Apply** | `spec` (scale) + `prototype` (display-heading treatment) |
| P5.2 | **Two spacing bands.** Airy daily register (non-home daily surfaces): card padding `24–32px`; section gap `32–48px > intra-card gap 12–16px`; prose measure `65–75ch`. Dense diagnostics register (rich home, `/health` detail, `/analytics` tables, node drill-down): **20px card padding / 28px section gap / 14px intra-card gap / 20px bento gutters**; section gap > intra-card gap preserved (28 > 14); prose ≤70ch. | **Apply** (amended) | `spec` (numbers locked for gates) + `prototype` |
| P5.3 | **Split into Today-cap (history) + rich-home cap.** Old ≤4 Today-cap retained as history, not applied to home. Rich-home cap **≤8 blocks (hero + ≤7 cards), one CTA, ≤1 grouped-count table, disclosures off the primary path.** | **Apply** (amended) | `spec` + `prototype` |
| P5.4 | **Palette fully tokenised**: no hex literals outside `:root`; **one accent**; muted semantic colours for exactly the five node states plus attention/warn/err; alias classes collapsed to one name each (`warn`, `err`); dead CSS removed (`header h1`/`.sub`, the four `960px` breakpoints under desktop-only). Light-first; dark mode stays out of scope — full tokenisation is what makes it a cheap token swap later instead of a redesign. | **Apply** | `spec` |
| P5.5 | **Hue, rail, modal treatment** are **not** decided here. Shell width (1040px/1120px for rich home), breakpoints (one 960px), bento-vs-single-column (rich home uses bento under dominance test) are **decided in G-Direction #248**. | — | `downstream` G-Direction + prototype briefs |

---
## D. Route surface & interaction posture (G-RouteSurface #220 · G-JS #225 · amended by G-JS #249)
...
- **Queue-row brief expansion — REFUSED, links.** Warrant: P-Unified marks it WANTS-JS but degrades to the `/next` stop, which already renders the five; nothing demonstrates the in-place form mattering.
- **Spine-chain / pathway fetch-refresh — REFUSED, links.** Warrant: P-GraphSpine + P-Unified both mark chain expansion WANTS-JS degrading to links; reconsider's "what unlocks what" was already an affordance upgrade, not a requirement.
- **Week-strip paging — REFUSED, links.** Warrant: P-Unified + reconsider plan variant mark prev/next-week WANTS-JS degrading to links; the week strip + one line reads complete without it.
- **Browse live filter — REFUSED, anchors.** Warrant: P-Unified marks filter-by-track/state WANTS-JS degrading to anchor links per track; the grouped-count table (the one blessed P2.1 borrowing) is the tier-0 form.
- **Real trend lines — GRANTED as static SVG + hover/focus tooltips only.** Warrant: the candidate list names real trend lines; R3's seam (`analytics/sparkline.py`, real multi-point series only per P2.2) is the renderer and tooltips are the one affordance static SVG cannot supply. No axes-at-scale, no zoom/pan — R3's tier-2 wall stays out.
- **Days-practiced heatmap — REFUSED, stays behind locked P1.7.** Warrant, three times over: P1.7 reads "no target, no countdown, no heatmap"; CONTEXT.md *Days practiced* is "a mirror, not a metronome"; a density grid is a streak/loss-framing device. Returns only as a fresh effort that re-opens the preference table per map #208's rule — not a v2.4 option.
- **Flash-without-URL-carry — REFUSED, URL carry kept.** Warrant: P3.5's handoff offers `history.replaceState` cleanup, but the owner kept `notice`/`kind` with the P3.5 copy rules and plain-link dismissal; #225's hard rules (URL is a copy surface; no URL-level redaction) stand.

**Charts — static SVG + hover/focus tooltips only:** server-rendered inline SVG through the existing `analytics/sparkline.py` seam, **real multi-point series only** (P2.2 bans pseudo-sparklines); hover/focus tooltips are the single granted interaction; no axes-at-scale, no zoom/pan — R3's tier-2 wall stays out.

**Flash-in-URL carry — kept.** `notice`/`kind` stay in the query string exactly as §C ratified them (names unchanged — the URL is not a design surface). Two hard rules: (1) **the URL is a copy surface, so P3.5 covers it** — every carried notice is already short human copy (no flags, command names, exit classes, paths, record ids or ADR numbers; today's notices are verbatim CLI stdout — the P3.5 translation module is the fix, and the address bar sits inside its remit); (2) **no URL-level redaction, ever** — copy is fixed at its source by the one translation module (P3.1), never stripped at the address bar. Idempotence from the 303 contract: a bare GET re-renders the same banner and never re-submits the write. **Dismissal** is a plain link to the current path without the query string (mechanism locked; wording per P3.5).
...
| Health / `/health` (A-Health #212) | **change** — de-nav-ed to the ambient line, detail on demand; **attention-first triage** (G-Health #251) | P2.3 ambient headline (three-state: "Everything looks good." / "Needs attention — N warning(s)." / "Needs attention — N layer(s) failing.") plus attention pill; per-layer detail **collapsed by default** inside `Full detail` disclosure; five rows in learner words: "Skills and links (graph) — 81 skills tracked, linked together", "Evidence (evidence) — 3 accepted pieces across 2 skills, 1 attempt recorded", "Study history (execution) — 5 study sessions, 12 log entries", "Study settings (policy) — 5 settings files, all readable", "Study materials (resources) — 29 materials, 2 need a look"; table headers `Area / What was checked / Status / To look at`; verdict line `health: OK/FAILED` dropped from web (CLI-only); liveness/actions as human lines (no command names); `--state` stays CLI-only; bare engine nouns as labels removed (P3.1). |
...
| **S3** | Unified home | The unified single-page home (§A) — hero focus viewport (pronoun CTA, named once, double-weight), six-card bento at dense register: queue (ranked preview rows), pressure (calm line), spine (pronoun-headed, named-twice), week (strip + honest blanks), session history (empty-state + format preview), browse (one blessed grouped-count table). Bento discipline: every bento card links-only, hero is the page's only CTA. | 7 blocks (hero + 6 bento; ≤8 cap), exactly one primary CTA page-wide, ≤1 grouped-count table, disclosures off primary path; dense band 20/28/14/20 locked; bento gutters 20px; section gap > intra-card gap preserved; focus named once; celebration past-tense/factual at real events only (CSS-only keyframe); `pytest` green. |
| **S4** | Next / node / finder | `/next` — human controls, candidate titles become links, one legible reasoning layer ("Why this?"), "Not ready yet — and why" card; node detail — brief-first state-aware collapse (P2.5), no-op evidence form omitted (P2.6), spine path summaries (§A), the tolerated raw-id as muted secondary + the single mono use (§B/E); title-first finder on `/nodes/jump` (absent param → title-first list) **enhanced with single-line node descriptions grouped by track/relation** (G-Health #251 handoff triaged here — fits frozen route table); **fix #228** (graph-edge links built from ids not titles). | `--state` never surfaces as a flag/web filter (recorded non-route); no command names/flags/raw-record ids in any prose/label/breadcrumb/bookmark; candidate titles are links; the finder is naive-first and server-rendered; #228's 404s are gone; `pytest` green. |
...
- **ADR 0007** — the architecture amendment (downward-only route gate, usable Vocabulary identities, structural/judgment request-time gating) already landed on `main` (commit `8d6ae8d`, G-RouteSurface #220); this spec renders it. **ADR 0008** is **written** (`docs/adr/0008-inline-progressive-enhancement-posture.md`, claims the reserved slot): narrow tier-1 shape, the single granted class (chart hover/focus tooltips), degradation contract, mutation safety, per-route budget gate. **ADR 0006 stands unamended** (G-JS #249).
- **spec-tier1-serve.md §C** — the stale sentence "the JavaScript-budget decision (ADR 0008, G-JS) remains open and unclaimed by it" is replaced with the locked answer (narrow tier 1: chart tooltips only; ADR 0008 written; ADR 0006 unamended; per-route budget gate replaces the no-`<script>` grep gate). §B is already aligned (structural vs judgment) and the normative route table is untouched.
...
- [ ] This spec plus the five throwaway prototypes (`p24-evolve`, `p24-radical`, `p24-reconsider`, `p25-graph-spine`, `p25-unified`) are linked from map #208's `## Decisions so far`, and this file is the hand-off artifact.
- [ ] No product, architecture, or UX decision remains that would block the five S1–S5 build slices (the S3 Unified home shape, S4 finder, S5 panels and charts all cited above by their locked source).
- [ ] Map #208's `## Not yet specified` is empty; its `## Out of scope` records the refused/closed patches (dark mode, mobile/PWA, graph-viz tech, CLI voice, new engine data, post-destination implementation, interactive charts beyond tooltips + the days-practiced heatmap).
- [ ] The rewritten v2.4 slot row, the §C sentence fix, and (where newly introduced) any glossary terms land in the same change.
- [ ] `pytest` green; `skilltrace health`/`today`/`next --minutes 60`/`node <seed id>` exit 0 on seed; doc gates green; the per-route budget gate (DD6 amended) is part of v2.4 acceptance.

---

## References

`CONTEXT.md` (glossary: *Days practiced*, *Next-action fact*, *Mentor*, *Interface sublayer*), `docs/adr/0006-stdlib-only-serve-shell.md`, `docs/adr/0007-reintroduce-interface-layer.md` (amended), `docs/adr/0008-inline-progressive-enhancement-posture.md`, `docs/spec-tier1-serve.md` §B/§C/§L-bis (normative route table), `docs/POST_V2_ROADMAP.md` v2.4 slot, map #208 + its child tickets (audits #209–#215, R3 `docs/research/r3-interaction-ceiling.md`, G-Preferences #219, G-RouteSurface #220, G-Direction #224, G-JS #225, G-Slot #226, G-MentorVoice #229, **map #243 + its child tickets: A-Landed #244, P-GraphSpine #245, P-Unified #246, G-Preferences #247, G-Direction #248, G-JS #249, G-Spec #250, G-Health #251**), `docs/SAFETY_BOUNDARIES.md`, `AGENTS.md` hard boundaries, prototypes `prototype/p24-*.html`, `prototype/p25-*.html` (throwaway decision aids).