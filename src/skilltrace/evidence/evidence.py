...
from .gate_receipt import (
    ...
    GATE_RUN_KEY,
    GATE_RUN_REQUIRED,
    HASH_PREFIX as _RECEIPT_HASH_PREFIX,
)
from .gate_receipt import validate as _validate_receipt

_HASH_PREFIX = _RECEIPT_HASH_PREFIX
...
def _check_gate_run(data: dict, ident: str, where: str) -> dict | None:
    ...
    Thin adapter over `gate_receipt.validate`: present on the record (even
    as null) it must be a well-typed receipt mapping. A *missing* `gate_run`
    is valid — pre-v2.2 and manual records carry none.
    """
    if GATE_RUN_KEY not in data:
        return None
    return _validate_receipt(data[GATE_RUN_KEY], ident=ident, where=where)
