...
def add_parser(subparsers: argparse._SubParsersAction) -> None:
    """Attach the `resource-report` parser to the top-level `subparsers` (issue #204).

    Co-located owner of the `resource-report` argparse surface; `cli.build_parser`
    calls this for the new path and skips its legacy `resource-report` block.
    """
